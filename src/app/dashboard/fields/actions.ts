...
const fieldSchema = z.object({
  ...
  type: z.enum(["Text", "Dropdown", "Radio", "Checkbox", "Number", "Textarea", "Date", "Label", "Time", "Price", "Days"]),
  required: z.boolean(),
...
export async function getFields(): Promise<Field[]> {
  try {
    ...
    let query = 'SELECT f.*, c.name as companyName FROM fields f JOIN companies c ON f.company_id = c.id ORDER BY f.id DESC';
